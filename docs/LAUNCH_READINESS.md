...
> - **E-posta:** `src/services/email_service.py` — Resend, SDK'sız düz HTTPS; `RESEND_API_KEY` yokken e-posta gönderilmez, motor loguna yalnızca gönderimin atlandığı bilgisi düşer (sıfırlama bağlantısı ve gövde asla loglanmaz — canlı token sızıntısı olurdu); akış dev ortamında da uçtan uca çalışır, geliştirmede bağlantıyı logda görmek için `EMAIL_DEV_LOG_LINK=true` gerekir (yalnızca yerel!). Hoş geldin + şifre sıfırlama şablonları hazır.
